...
function bp_dtheme_ajax_invite_user() {
	...
	if ( 'invite' == $_POST['friend_action'] ) {
		...
		echo '<div class="action">
				<a class="btn btn-small remove" href="' . wp_nonce_url( $bp->loggedin_user->domain . bp_get_groups_slug() . '/' . $_POST['group_id'] . '/invites/remove/' . $user->id, 'groups_invite_uninvite_user' ) . '" id="uid-' . esc_attr( $user->id ) . '">' . __( 'Remove Invite', 'cc' ) . '</a>
			  </div>';
		echo '</li>';
